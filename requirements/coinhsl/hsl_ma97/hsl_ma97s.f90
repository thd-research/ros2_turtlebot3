...
!$    integer(omp_lock_kind) :: lock
   ...
   type ma97_fkeep
      ...
      integer :: matrix_rank
      integer :: maxfront ! maximum number of rows in a supernode
      integer :: maxsupernode ! maximum number of columns in a supernode
      integer :: num_delay
   ...
   type thread_stats
      ...
      integer :: st = 0
      integer :: maxfront = 0 ! Max front size (max no. rows in a supernode)
      integer :: maxsupernode = 0 ! max no. columns in a supernode
   ...
   type ma97_info
      ...
      integer :: maxfront ! Maximum front size (max no. rows in a supernode)
      integer :: maxsupernode ! max no. columns in a supernode
      integer(long) :: num_factor = 0_long ! Number of entries in factors
      integer(long) :: num_flops = 0_long ! Number of floating point operations
      integer :: num_delay = 0 ! Number of delayed variables
      integer :: num_neg = 0 ! Number of negative pivots
...
subroutine analyse_phase(n, ptr, row, ptr2, row2, order, invp, perm,  &
   ...
   do i = akeep%nnodes, 1, -1
      ...
      info%maxfront = max(info%maxfront, blkm)
      info%maxsupernode = max(info%maxsupernode, blkn)
...
!$OMP END PARALLEL
   ...
   info%maxsupernode = maxval(stats(:)%maxsupernode)
...
!$OMP END PARALLEL
   ...
   if (control%print_level >= 1 .and. control%unit_diagnostics >= 0) then
      ...
      write (control%unit_diagnostics, &
         ...
         info%maxfront, &
         ' maxsuperpnode          Maximum supernode size                   = ',&
         info%maxsupernode, &
   ...
   fkeep%maxsupernode = info%maxsupernode
...
!$ this_thread = omp_get_thread_num() + 1
   ...
   do node = sa, en
      ...
      stats(this_thread)%maxfront = max(stats(this_thread)%maxfront, blkm)
      stats(this_thread)%maxsupernode = &
         max(stats(this_thread)%maxsupernode, blkn)
...
subroutine ma97_solve_mult_single(nrhs, x, ldx, akeep, fkeep, control, &
   ...
   info%maxsupernode = fkeep%maxsupernode

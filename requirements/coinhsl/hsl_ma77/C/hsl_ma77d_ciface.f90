...
subroutine ma77_solve_fredholm_d(nrhs, cflag_out, lx, cx, ckeep, ccontrol, &
      cinfo, cscale) bind(C)
   use hsl_ma77_double_ciface
   implicit none
   ...
   elsewhere
      fiflag_out(:) = 0 ! False
   endwhere
end subroutine ma77_solve_fredholm_d

subroutine ma77_lmultiply_d(ctrans, k, lx, cx, ly, cy, ckeep, ccontrol, cinfo, &
      cscale) bind(C)
   use hsl_ma77_double_ciface
   implicit none
   ...
   logical :: ftrans
   real(C_DOUBLE), dimension(:,:), pointer :: fx
   real(C_DOUBLE), dimension(:,:), pointer :: fy
   type(f_ma77_keep), pointer :: fkeep
   type(f_ma77_info) :: finfo
   type(f_ma77_control) :: fcontrol
   logical :: f_arrays
   ...
   ! Copy data out
   call copy_info_out(finfo, cinfo)
end subroutine ma77_lmultiply_d
